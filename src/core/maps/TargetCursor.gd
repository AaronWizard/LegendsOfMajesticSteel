...
export var rect_size: Vector2 = Vector2(1, 1) setget set_rect_size
...
func set_rect_size(new_value: Vector2) -> void:
	rect_size = new_value
	_arrange_corners()
...
func _arrange_corners() -> void:
	...
	if centered:
		var half_size := (rect_size / 2) * Constants.TILE_SIZE
		...
		var size := rect_size * Constants.TILE_SIZE
